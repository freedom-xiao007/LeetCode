...
class Solution:
    def displayTable(self, orders: List[List[str]]) -> List[List[str]]:
        ...
        for order in orders:
            ...
            if tableNum not in menu:
                menu[tableNum] = {}
                menu[tableNum][food] = 1
            else:
                if food not in menu[tableNum]:
                    menu[tableNum][food] = 1
                else:
                    menu[tableNum][food] = menu[tableNum][food] + 1

        sortedFods = sorted(foods)
        ans = []
        ans.append(["Table"] + sortedFods)
        for table in sorted(menu.keys()):
            record = [str(table)]
            for food in sortedFods:
                if food not in menu[table]:
                    record.append("0")
                else:
                    record.append(str(menu[table][food]))
            ans.append(record)
